...
底盘 `x/y/yaw` 使用四主动轮对角线交点，不使用底盘几何中心。四主动轮对角线交点相对底盘几何中心为 `(-0.035m, 0.0m)`。
...
arm_action：path 输出 h/q1/q2/gripper_yaw/gripper_opening；速度前馈只包含 h/q1/q2
...
kind=flags：占用 flags 控制，包括上漏斗、下漏斗。
```
...
connection/
  ...
  protocol.py       14 float + flags + CRC-8 二进制协议
```
...
`gripper_yaw/gripper_opening` 是舵机目标角，允许目标角跳变，不参与 arm TOPPRA 的速度/加速度约束，也不下发速度前馈。舵机实际角由舵机或下位机按自身速度限制追踪目标；仿真端也按该语义限速追踪。Arm path 的 `duration` 至少覆盖舵机按速度上限追到最后目标所需时间，纯舵机动作也会占用 `kind=arm` 一小段时间。
