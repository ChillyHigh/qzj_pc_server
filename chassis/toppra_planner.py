...
def direct(start: Pose, end: Pose) -> AbstractGeometricPath:
    """底盘从 start 移动到 end，自动避障。

    先检查直线路径是否无碰撞；无碰撞则走最短直线，否则用 Theta* 规划绕行路径。
    """
    try:
        validate_chassis_path(start, end)
        unwrapped_end = _unwrap_end_yaw(start, end)
        waypoints = [
            Waypoint(q=start, source_kind="segment", source_id=0),
            Waypoint(q=unwrapped_end, source_kind="segment", source_id=1),
        ]
    except ValueError:
...
def s_cross(start: Pose, end: Pose) -> AbstractGeometricPath:
