...
def test_parse_feedback_accepts_valid_crc_frame() -> None:
    frame = _feedback_frame((1.0, 2.0, 45.0, 4.0, 90.0, 180.0))

    feedback, consumed = parse_feedback(frame)

    assert feedback == Feedback(1.0, 2.0, math.pi / 4.0, 4.0, math.pi / 2.0, math.pi)
...
def test_consume_feedback_bytes_counts_bad_crc_and_continues() -> None:
    ...
    bad[7] ^= 0x01
    good = _feedback_frame((7.0, 8.0, 45.0, 10.0, 90.0, 180.0))
    ...
    assert drops == 1
    assert published == [Feedback(7.0, 8.0, math.pi / 4.0, 10.0, math.pi / 2.0, math.pi)]
